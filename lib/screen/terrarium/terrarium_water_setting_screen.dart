...
class TerrariumWallWaterSettingScreen extends HookConsumerWidget {
  const TerrariumWallWaterSettingScreen({super.key});
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    useMemoized(() {
      ...
      periodTime.value = DateTime.parse("2023-06-09 ${data.vaporizerPeriod}");
      workingTime.value = DateTime.parse("2023-06-09 ${data.vaporizerTime}");
      isAuto.value = data.vaporizerAuto;
      isOn.value = data.vaporizerOn;
    });
    return Scaffold(
      ...
      body: Column(
        ...
        children: [
          const Text("안개 분무 설정").textStyle(titleStyle),
          ...
          GreenButton(
            ...
            onPressed: () async {
              ...
              // var string = "F1000";
              var newStartTime = "VT" + DateFormat("HHmm").format(workingTime.value);
              await cha?.write(newStartTime.codeUnits, withoutResponse: true);
              var newEndTime = "VP" + DateFormat("HHmm").format(periodTime.value);
              await cha?.write(newEndTime.codeUnits, withoutResponse: true);
              var newisAuto = "VA" + (isAuto.value ? '1' : '0');
              await cha?.write(newisAuto.codeUnits, withoutResponse: true);
              var newisOn = "VO" + (isOn.value ? '1' : '0');
